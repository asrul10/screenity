$(document).ready(function () {
  ...
  function injectCode(inject, active) {
    if (inject) {
      ...
      var iframeinject =
        "<div id='camera-hide'></div><div id='wrap-iframe-camera'><iframe src='" +
        chrome.extension.getURL("./html/camera.html") +
        "' allow='camera;microphone'></iframe></div>";
      $("#" + uniqueid).prepend(iframeinject);

      // Inject the toolbar
      var toolbarinject =
        "<iframe id='toolbar-settings-camera' class='toolbar-inactive-camera' src='" +
        chrome.extension.getURL("./html/settings.html") +
        "'></iframe><div id='toolbar-record-camera'><div id='pause-camera' class='tool-camera'><img src='" +
        chrome.extension.getURL("./assets/images/pausewhite.svg") +
        "'/></div><div id='mic-camera' class='tool-camera tool-active-camera'><img src='" +
        chrome.extension.getURL("./assets/images/mic-off.svg") +
        "'/></div><div id='settings-camera' class='tool-camera'><img src='" +
      ...
      $("#" + uniqueid + " #wrap-iframe-camera iframe").on("load", function () {
        ...
        if (active) {
          $("#" + uniqueid + " #toolbar-record-camera").css(
            "pointer-events",
            "none"
          );
  ...
  function injectCountdown(time) {
    var countdowninject =
      "<div id='countdown-camera'><img src='" +
  ...
  function delay(num, time, last) {
    window.setTimeout(function () {
      if (!last) {
        $("#" + uniqueid + " #countdown-camera img").attr(
          "src",
          chrome.extension.getURL("./assets/images/" + num + "-countdown.svg")
        );
      } else {
        $("#" + uniqueid + " #countdown-camera").addClass(
          "countdown-done-camera"
        );
        chrome.runtime.sendMessage({ type: "countdown" });
        recording = true;
        $("#" + uniqueid + " #toolbar-record-camera").css(
  ...
  function pauseRecording() {
    recording = false;
    $("#" + uniqueid + " #pause-camera img").attr(
      "src",
  ...
  function resumeRecording() {
    ...
    $("#" + uniqueid + " #pause-camera img").attr(
      "src",
      chrome.extension.getURL("./assets/images/pause-white.svg")
    );
    $("#" + uniqueid + " #mic-camera img").attr(
      "src",
      chrome.extension.getURL("./assets/images/mic-off.svg")
  ...
  function hideTools() {
    if (!persistent) {
      ...
      $("#" + uniqueid + " #toolbar-record-camera").addClass(
        "toolbar-inactive-camera"
      );
    }
  }
  ...
  $(document).on(
    "click",
    "#" + uniqueid + " #toolbar-record-camera #settings-camera",
    function () {
      if (recording) {
        if (
          $(
            "#" + uniqueid + " #toolbar-record-camera #settings-camera"
          ).hasClass("pen-options-camera")
        ) {
          moretools = false;
          $(
            "#" + uniqueid + " #toolbar-record-camera #settings-camera"
          ).removeClass("pen-options-camera");
          $(
            "#" + uniqueid + " #toolbar-record-camera #settings-camera img"
          ).attr(
            "src",
            chrome.extension.getURL("./assets/images/settings.svg")
          );
          ...
          sendSettings();
          moretools = true;
          $("#" + uniqueid + " #toolbar-settings-camera").removeClass(
            "toolbar-inactive-camera"
          );
          $(
            "#" + uniqueid + " #toolbar-record-camera #settings-camera"
          ).addClass("pen-options-camera");
          $(
            "#" + uniqueid + " #toolbar-record-camera #settings-camera img"
          ).attr("src", chrome.extension.getURL("./assets/images/close.svg"));
        }
      } else {
        cancelRecording();
      }
